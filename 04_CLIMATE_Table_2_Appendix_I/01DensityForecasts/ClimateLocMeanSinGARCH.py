...
"""
Purpose:
    Estimation and prediction LocalMeanSinGARCH 
"""
...
###########################################################
def TempARGARCHLink(vParams, sDistr='normal'):
    """
    Purpose
    ----------
    Function of parameters, freely doubleing to transformed (tilde) parameters

    Parameters
    ----------
    vParams         vector, parameters for TempARGARCH model
    ...
    # dBeta [logit]
    vParamsTrOut[5] = logit(vParamsCopy[5])  
...
###########################################################
def TempARGARCHLinkInverse(vParamsTr, sDistr='normal'):
    """
    Purpose
    ----------
    ...
    Parameters
    ----------
    vParamsTr       transformed parameters for TempARGARCH model
    ...
    # dBeta [logit]
    vParamsOut[5] = expit(vParamsTrCopy[5])  
...
###########################################################
def TempARGARCHAvgNLLTr(vParamsTr, vY, vT, sDistr):
    """
    Purpose
    ----------
    ...
    Parameters
    ----------
    vParamsTr       vector, transformed parameters for TempARGARCH model
    ...
    vParamsTrCopy = np.copy(vParamsTr)
    vParamsCopy = TempARGARCHLinkInverse(vParamsTrCopy, sDistr)
    return TempARGARCHAvgNLL(vParamsCopy, vY, vT, sDistr)

###########################################################
def TempARGARCHAvgNLL(vParams, vY, vT, sDistr='normal'):
    """
    Purpose
    ----------
    Calculate the average negative log-likelihood for the TempARGARCH model
    ...
    # Initialization 
    iT = len(vY)
    dMu, dPhi, dOmega0, dOmega1, dAlpha, dBeta =  vParams[:6]
    if sDistr == 't':
        dNu = vParams[-1]
    ...
    for t in range(1, iT):
        ...
        vSig2[t] = dOmega0  + dOmega1 * vSinT[t] + dAlpha * (vEps[t - 1])**2 + dBeta * vSig2[t - 1]
...
###########################################################
def TempARGARCHEstim(vY, vT, sDistr='normal', bWarm=False):
    """
    Purpose
    ----------
    Estimate Temp-AR-GARCH model parameters by maximum likelihood
    ...
    if sDistr == 'normal':
        vParamsInit = np.array([dMuInit,dPhiInit,dOmega0Init,dOmega1Init,dAlphaInit,dBetaInit])
    elif sDistr == 't':
        vParamsInit = np.array([dMuInit,dPhiInit,dOmega0Init,dOmega1Init,dAlphaInit,dBetaInit,dNuInit])

    vParamsInitTr = TempARGARCHLink(vParamsInit, sDistr)
                    
    oResult = minimize(lambda x: TempARGARCHAvgNLLTr(x, vY, vT, sDistr), vParamsInitTr, method='L-BFGS-B') #L-BFGS-B
    vParamsOptTr = np.copy(oResult.x)
    
    vParamsOpt = TempARGARCHLinkInverse(vParamsOptTr, sDistr)
    dLLOpt = -TempARGARCHAvgNLL(vParamsOpt, vY, vT, sDistr) * (len(vY)-1)
...
###########################################################
def GARCHTempForecast(vY, vT, vTh, vParams, sDistr):
    """
    Purpose
    ----------
    ...
    vSig2Pred[0] = vSig2[-1] # sig2_T
    
    for h in range(1,iHorizon+1):
        vSig2Pred[h] = dOmega0 + dOmega1 * np.abs(np.sin(np.pi/365 * np.max((vTh[h-1], 365)))) + (dAlpha + dBeta) * vSig2Pred[h-1] 
       
    return vSig2Pred[1:]

###########################################################
def GARCHEstimAndForecast(vY, vT, vTh, sDistr, bWarm=False):
    """
    Purpose
    ----------
    ...
    # Estimate parameters
    vParamsHat, dLL, vYNormalized, vDailyAveragesExpanded = TempARGARCHEstim(vY, vT, sDistr)
    
    # Collect forecasts
    mParamsDF[:,0] = ARTempForecast(vYNormalized, vDailyAveragesExpanded, vTh, vParamsHat[:2])  
    mParamsDF[:,1] = GARCHTempForecast(vYNormalized, vT, vTh, vParamsHat, sDistr)
