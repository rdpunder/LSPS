...
"""
Purpose:
    Estimation and prediction LocalMeanSinGARCHI
"""
...
###########################################################
def TempQGARCHILink(vParams, sDistr='normal'):
    """
    Purpose
    ----------
    ...
    Parameters
    ----------
    vParams         vector, parameters for TempQGARCHI model
    ...
    # dGamma0, dGamma1
    vParamsTrOut[5:7] = vParamsCopy[5:7]

    # dBeta [logit]
    vParamsTrOut[7] = logit(vParamsCopy[7])  
...
###########################################################
def TempQGARCHILinkInverse(vParamsTr, sDistr='normal'):
    """
    Purpose
    ----------
    ...
    Parameters
    ----------
    vParamsTr       transformed parameters for TempQGARCHI model
    ...
    # dGamma0, dGamma1, dGamma2
    vParamsOut[5:7] = vParamsTrCopy[5:7]

    # dBeta [logit]
    vParamsOut[7] = expit(vParamsTrCopy[7])  
...
###########################################################
def TempQGARCHIAvgNLLTr(vParamsTr, vY, vT, sDistr):
    """
    Purpose
    ----------
    Wrapper for calculating the average negative log-likelihood for the TQGARCHI model

    Parameters
    ----------
    vParamsTr       vector, transformed parameters for TempQGARCHI model
    ...
    vParamsTrCopy = np.copy(vParamsTr)
    vParamsCopy = TempQGARCHILinkInverse(vParamsTrCopy, sDistr)
    return TempQGARCHIAvgNLL(vParamsCopy, vY, vT, sDistr)

###########################################################
def TempQGARCHIAvgNLL(vParams, vY, vT, sDistr='normal'):
    """
    Purpose
    ----------
    Calculate the average negative log-likelihood for the TempQGARCHI model

    Parameters
    ----------
    vParams         vector, all parameters for AR(p)-TQGARCHI model
    vY              vector, time series data
    vT              vector, time values
    sDistr          string, distribution for the innovations (normal or t)
    ...
    # Initialization
    iT = len(vY)
    dMu, dPhi, dOmega0, dOmega1, dAlpha, dGamma0, dGamma1, dBeta =  vParams[:8]
    if sDistr == 't':
        dNu = vParams[-1]
    ...
    for t in range(1, iT):
        ...
        vSig2[t] = dOmega0  + dOmega1 * vSinT[t] + dAlpha * (vEps[t - 1] - dGamma0 - dGamma1 * vSinT[t])**2 + dBeta * vSig2[t - 1]
...
###########################################################
def TempQGARCHIEstim(vY, vT, sDistr='normal', bWarm=False):
    """
    Purpose
    ----------
    Estimate the AR(p)-TQGARCHI model parameters: 
    ...
    # Use TempGARCH for starting values if bWarm is true
    if bWarm:
      vParamsInit, dLLGARCH, vYNormalized, vDailyAveragesExpanded =  TempARGARCHEstim(vY, vT, sDistr)
      dMuInit,dPhiInit,dOmega0Init,dOmega1Init,dAlphaInit,dBetaInit = vParamsInit[:6]
      if sDistr == 't':
        dNuInit = vParamsInit[-1]
        
    if sDistr == 'normal':
        vParamsInit = np.array([dMuInit,dPhiInit,dOmega0Init,dOmega1Init,dAlphaInit,dGamma0Init,dGamma1Init,dBetaInit])
    elif sDistr == 't':
        vParamsInit = np.array([dMuInit,dPhiInit,dOmega0Init,dOmega1Init,dAlphaInit,dGamma0Init,dGamma1Init,dBetaInit,dNuInit])

    vParamsInitTr = TempQGARCHILink(vParamsInit, sDistr)
                    
    oResult = minimize(lambda x: TempQGARCHIAvgNLLTr(x, vY, vT, sDistr), vParamsInitTr, method='L-BFGS-B') #L-BFGS-B
    vParamsOptTr = np.copy(oResult.x)
    
    vParamsOpt = TempQGARCHILinkInverse(vParamsOptTr, sDistr)
    dLLOpt = -TempQGARCHIAvgNLL(vParamsOpt, vY, vT, sDistr) * (len(vY)-1)
...
###########################################################
def QGARCHITempForecast(vY, vT, vTh, vParams, sDistr):
    """
    Purpose
    ----------
    ...
    for h in range(1,iHorizon+1):
        vSig2Pred[h] = dOmega0 + dOmega1 * np.abs(np.sin(np.pi/365 * np.min((vTh[h-1], 365)))) + (dAlpha + dBeta) * vSig2Pred[h-1] + dAlpha * (dGamma0 + dGamma1 * np.abs(np.sin(np.pi/365 * np.min((vTh[h-1], 365)))))**2
    
    return vSig2Pred[1:]

###########################################################
def QGARCHIEstimAndForecast(vY, vT, vTh, sDistr, bWarm=False):
    """
    Purpose
    ----------
    ...
    # Estimate parameters
    vParamsHat, dLL, vYNormalized, vDailyAveragesExpanded = TempQGARCHIEstim(vY, vT, sDistr, bWarm)
    
    # Collect forecasts
    mParamsDF[:,0] = ARTempForecast(vYNormalized, vDailyAveragesExpanded, vTh, vParamsHat[:2]) 
    mParamsDF[:,1] = QGARCHITempForecast(vYNormalized, vT, vTh, vParamsHat, sDistr)
