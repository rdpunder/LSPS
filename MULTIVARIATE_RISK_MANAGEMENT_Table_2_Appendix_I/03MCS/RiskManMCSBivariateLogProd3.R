# Model confidence set - test in R
...
# Load datafiles for h=1 and h=5
iTest = 1000
mLossh1 = -np$load('mScores/mScores_h1_LogProd3DCC_iT_6218_iTest_1000_vTGFinalXLEF_iP_0_p_0_qml_0_iTw_0_dRq_25_iS_16_vFinalXLEF.npy')
mLossh5 = -np$load('mScores/mScores_h5_LogProd3DCC_iT_6218_iTest_1000_vTGFinalXLEF_iP_0_p_0_qml_0_iTw_0_dRq_25_iS_16_vFinalXLEF.npy')

## Parameters MCS procedure
sStatistic = 'TR' # statistic, 'Tmax' or 'TR'
bCustom = TRUE
iB = 10000  # number of bootstrap replications
iK= 5     # block length [report for 20 and 100]


# Define the number of cores
iCores = 5#detectCores()
#iQ = 3
set.seed(1634)
...
MCSpvalCustomRiskMan <- function(mLossh1, mLossh5, iB, sStatistic, iK){
 ...
 #'  description
  ...
  lScoringRules = c('LogSflat', 'LogSsharp','LogSsharpslog','LogSsharpsbar','QSflat', 'QSsharp', 'QSsharpslog', 'QSsharpsbar', 'SphSflat', 'SphSsharp','SphSsharpslog','SphSsharpsbar', 'CRPSflat', 'CRPSsharp','CRPSslog','CRPSsbar','LogSflat5', 'LogSsharp5','LogSsharpslog5','LogSsharpsbar5','QSflat5', 'QSsharp5', 'QSsharpslog5', 'QSsharpsbar5', 'SphSflat5', 'SphSsharp5','SphSsharpslog5','SphSsharpsbar5', 'CRPSflat5', 'CRPSsharp5','CRPSslog5','CRPSsbar5')
  ...
  for(h in 1:length(vH)){
    ...
    for(i in 1:length(vIdx)){
      ...
      while(dAlpha < 1 & ncol(dfLossS) > 1){
        ...
        set.seed(1634) # still varying [very modestly] over different runs, to deep in package code 
...
for(q in 1:length(vQ)){
    ...
    sFileName = paste('MCSTables/RiskManXLEFLogProd3_MCSTable_',as.character(sStatistic),'_iK',as.character(iK),'_iTest', as.character(iTest),'_q', as.character(vQ[q]*100),'.xlsx',sep = "")
