...
**Dependencies**: Code is written in `Python 3.9.18` unless we build on existing `R 4.3.0` code.
...
For exact replication of our results, we recommend using the provided files in [01DensityForecasts/Data](02_MULTIVARIATE_RISK_MANAGEMENT_Table_2_Appendix_I/01DensityForecasts/Data). 
Re-downloading the series may lead to small numerical differences (~10e-7) due to real time auto-adjustments for dividends and stock splits and/or other differences related to (future) changes to the `yfinance` module. 
