...
The script `EmpiricalDataMultivariateRiskMan.py` located in the [DataConstruction](MULTIVARIATE_RISK_MANAGEMENT_Table_2_Appendix_I/01DensityForecasts/Data/DataConstruction) folder, downloads the price series, transforms them into log returns, and merges them with the correponding deannualized realized measures. The script produces the files `XLEandRealVolXiu.csv` and `XLFandRealVolXiu.csv`, which can then be copied to the [01DensityForecasts/Data](MULTIVARIATE_RISK_MANAGEMENT_Table_2_Appendix_I/01DensityForecasts/Data) folder. 
...
* [FIG_G6_RejRates_LP_L_C20](MONTE_CARLO_Appendix_G/FIG_G6_RejRates_LP_L_C20): Rejection rates for power study Laplace(-1,1) versus Laplace(1,1.1) where the region of interest is the center, with *c=20* observations in the region of interest. For reproduction, run
...
* [FIG_G7_LocalDiv_LP_L_C20](MONTE_CARLO_Appendix_G/FIG_G7_LocalDiv_LP_L_C20): Standardized local divergences for Laplace(-1,1) versus Laplace(1,1.1) where the region of interest is the center. For reproduction, run
