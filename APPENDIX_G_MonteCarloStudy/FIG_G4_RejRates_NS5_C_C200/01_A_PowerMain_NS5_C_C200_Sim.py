...
def main():    
    ...
    ## Goal
    iGoal = 1                   # 1: simulate data or calculate DM statistics, 1: plotting
    bLeftTail = False           # boolean, left tail indicator example if true, centre indicator if false
    sDensClass = 'StudentNorm'  # StudentNorm or Laplace
    bSimulateData = True
    
    ## Fundamentals
    iRtot = 200             # steps Rgrid 
    iRep = 10000            # number of Monte Carlo replications
    
    iAxis = 0               # 0: threshold r
    iC = 200                # expected number of observations in {w>0} 
    dLeftTailPerc =0.01     # left tail percentile [c grid]
    
    dISWeightLB = 0.1       # obsverations assigned with weights below this threshold are included in IS
    ...
    dictScores = {
            ...
            'CRPSFlat':{'fScore': CRPSGenFlatGamma,'vParamsS': dictCRPSNumInt, 'bWeighted': True, 'Label': 'CRPS$^\\flat$', 'Colour': lCycle[1], 'Symbol': lStyles[0]},
            ...
            'twCRPS':{'fScore': twCRPS,'vParamsS': dictCRPSNumInt, 'bWeighted': True, 'Label': 'CRPS$^\\flat$', 'Colour': lCycle[4], 'Symbol': lStyles[0]},
            }
