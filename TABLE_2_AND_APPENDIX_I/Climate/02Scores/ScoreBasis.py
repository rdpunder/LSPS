...
###########################################################  
def RollingScoresMPI(dictMethods, dictScores, dictW, lY, lParamsDF, mRhat, vIntScoresByProc, dictNumIntSettings, mGammaHat=None):
    """
    Purpose
    ----------
    ...
    dictNumIntSettings   dictionary, numerical settings
            ...
            for m in range(iM):
                ...
                for i in range(len(vIntScoresByProc)):
                    ...
                    dictPreCalc = {'AlphaNormsF': {dictDistr['sDistr']: AlphaNormsF(dictDistr, dictW, np.array([2]))},
                                   ...
                                   'NumIntSettings': dictNumIntSettings,
                                   }
...
###########################################################  
def mRhatFixed(lRq, iNumEstWindows):
    """
    Purpose
    ----------
    mRhat construction for fixed bands

    Parameters
    ----------
    lRq                         list, emprical quantiles levels
    iNumberEstimationwindows    number of estimation windows
    
    Returns
    -------
    mRhat :                     matrix, fixed quantiles
    """
