...
if(bInflation){
  ...
  for(h in 1:length(lLoss)){
    ...
    for(q in 1:length(vQ)){
      ...
      sFileName = paste('MCSTables/Inflation_MCSTableC_','h',as.character(c(6,24)[h]),'_',as.character(sStatistic),'_iK',as.character(iK),'_iTest', as.character(iTest),'_q', as.character(vQ[q]*100),'_tails',as.character(as.integer(bTails)),'.xlsx',sep = "")
      write_xlsx(dfMCSpval, sFileName)
    }
  }
}
...
if(bClimate){
  ...
  for(h in 1:length(lLoss)){
    ...
    for(q in 1:length(vQ)){
      ...
      sFileName = paste('MCSTables/Climate_MCSTable_','h',as.character(c(1,3)[h]),'_',as.character(sStatistic),'_iK',as.character(iK),'_iTest', as.character(iTest),'_q', as.character(vQ[q]*100),'_tails',as.character(as.integer(bTails)),'.xlsx',sep = "")
      write_xlsx(dfMCSpval, sFileName)
    }
  }
}
